    ...
    async execute(interaction) {
        switch (interaction.options.getSubcommand()) {
            case 'status':
                ...
                break;
            case 'toggle':
                const scraper = yadBot.scrapers.find((scraper) =>
                    scraper.constructor.name
                        .toLowerCase()
                        .includes(interaction.options.getString('scraper-name').toLowerCase()),
                );
                        ...
                        embeds: [
                            {
                                ...
                                description: `${scraper.constructor.name} is already ${
                                    scraperWasActive ? 'active' : 'inactive'
                                }.`,
                        ...
                        embeds: [
                            {
                                ...
                                description: `${scraper.constructor.name} has been toggled ${
                                    scraperWasActive ? `inactive` : `active`
                                }.`,
                ...
                break;
            case 'notify':
                const targetScraper = yadBot.scrapers.find((scraper) =>
                    scraper.constructor.name
                        .toLowerCase()
                        .includes(interaction.options.getString('scraper-name').toLowerCase()),
                );
                ...
                if (targetScraper === undefined) {
                    yadBot.sendCommandErrorEmbed(
                        interaction,
                        `You need to provide additional arguments for this command or incorrect arguments were given.\n Use \`${config.prefix}help\` to get more information`,
                    );
                    return;
                }
                ...
                break;
            case 'send':
                const sendingScraper = yadBot.scrapers.find((scraper) =>
                    scraper.constructor.name
                        .toLowerCase()
                        .includes(interaction.options.getString('scraper-name').toLowerCase()),
                );
                ...
                const jsonData = files.readCompleteJson(
                    `${sendingScraper.getScraperEmbedPath()}/${interaction.options.getString('file-name')}`,
                );
                ...
                const embed = sendingScraper.filterEmbedLength(
                    sendingScraper.getEmbed(jsonData.data[jsonData.data.length - 1]),
                );

                // todo: add argument 'all', send to all subscribers then
                sendingScraper.log(
                    `Sending embed(s) to ${interaction.options.getChannel('target-channel').guild?.name}:${
                        interaction.options.getChannel('target-channel').name
                    }`,
                );
                interaction.options
                    .getChannel('target-channel')
                    .send({ embeds: [embed] })
                    .catch((e) => {
                        sendingScraper.errorLog(
                            `error with guild ${interaction.options.getChannel('target-channel').guild?.id} channel ${
                                interaction.options.getChannel('target-channel').id
                            }`,
                        );
                        ...
                        sendingScraper.sendMissingAccessToGuildAdmins(
                            interaction.options.getChannel('target-channel').guild.id,
                        );
                        console.dir(e);
                    });
                    ...
                    embeds: [
                        {
                            ...
                            description: `The embed has been sent to the specified channel ${interaction.options
                                .getChannel('target-channel')
                                .toString()}.`,
                            color: EmbedColors.GREEN,
                        },
            ...
            case 'list-subscribers':
                const subScraper = yadBot.scrapers.find((scraper) =>
                    scraper.constructor.name
                        .toLowerCase()
                        .includes(interaction.options.getString('scraper-name').toLowerCase()),
                );
