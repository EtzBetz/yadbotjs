...
export default {
    ...
    execute(interaction) {
        files.readJson(yadBot.getCommandConfigPath('notes'), interaction.user.id, false, []);
        ...
        switch (interaction.options.getSubcommand()) {
            ...
            case 'add':
                ...
                notes.forEach((entry, index) => {
                    notesString += ` \`${index + 1}:\` ${index + 1 === notes.length ? '**' : ''}${entry}${
            ...
            case 'remove':
                ...
                JSON.parse(oldNotes).forEach((entry, index) => {
                    notesString += ` \`${index + 1}:\` ${
                        index + 1 === interaction.options.getInteger('note-number') ? '**~~' : ''
                    }${entry}${index + 1 === interaction.options.getInteger('note-number') ? '~~**' : ''}\n`;
                });
                if (notesString === '') notesString = 'Your notes list is empty.';
