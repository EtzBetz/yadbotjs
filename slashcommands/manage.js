...
export default {
    ...
    getData() {
        return {
            name: 'manage',
            description:
                'Shows a list of all available scrapers or lets you subscribe to them via PM or server channel.',
            options: [
                {
                    name: 'subscribe',
        ...
        switch (interaction.options.getSubcommand()) {
            case 'subscribe':
                ...
                for (let i = 0; i < yadBot.scrapers.length; i++) {
                    if (
                        yadBot.scrapers[i].constructor.name
                            .toLowerCase()
                ...
                if (selectedScraper === undefined) {
                    yadBot.sendCommandErrorEmbed(
                        interaction,
                        `No scraper has been found for '${interaction.options
                            .getString('scraper-name')
                            .toLowerCase()}'`,
                    );
                    return;
                }
