...
class ScraperMakerSpaceEvents extends WebsiteScraper {
    ...
    async requestWebsite(url) {
        ...
        for (let i = 0; i < 6; i++) {
            let data = `limitMonth=${parseInt(dateTime.toFormat('M'), 10) - 1}&limitYear=${dateTime.toFormat(
                'y',
            )}&lastMonth=${parseInt(dateTime.toFormat('M'), 10) + 3}&lastYear=${dateTime.toFormat('y')}`;
        ...
        if (scrapeInfo.response !== undefined) {
            for (const response of scrapeInfo.response) {
                ...
                entities.forEach((entity, index) => {
                    ...
                    let entryFillState = entity.children[2]
                        ?.querySelector('div > div.pBar > div.level')
    ...
    async getEmbed(content) {
        ...
        return new Discord.EmbedBuilder({
            ...
            description: `Datum: **${content.json.date}**\nAnmeldungen: **${
                content.json.fillstate === undefined ? 'Gesperrt' : `${content.json.fillstate}%`
            }**`,
            author: {
