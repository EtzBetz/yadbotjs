...
class ScraperFreeSteamGames extends WebsiteScraper {
    getScrapingUrl() {
        const apiKey = files.readJson(
            this.getScraperConfigPath(),
            'steam_dev_api_key',
            true,
            'ENTER STEAM DEV API KEY HERE',
        );
    ...
    getLastScrapingUnixTime() {
        return files.readJson(
            this.getScraperConfigPath(),
            'last_scraping_unix_time',
            false,
            Math.floor(luxon.DateTime.local().minus({ minutes: 30 }).toSeconds()),
        );
    }
        ...
        if (JSON.stringify(scrapeInfo.response.data.response) !== '{}') {
            for (const game of scrapeInfo.response.data?.response?.apps) {
                ...
                if (game.price_change_number !== 0) {
                    ...
                    let detailResponse = await super.requestWebsite(
                    ...
                    if (detailResponse.data[game.appid.toString()].success === true) {
                        ...
                        if (
                            ...
                            entry.imageUrl = detailData.header_image.substring(
                                0,
                                detailData.header_image.indexOf('?') !== -1
                                    ? detailData.header_image.indexOf('?')
                                    : detailData.header_image.length,
                            );
    ...
    parseReleaseDate(dateString) {
        ...
        if (isoDate === null || !isoDate.isValid) {
            yadBot.sendMessageToOwner(
                `Date '${dateString}' with '${day}.${monthNumber + 1}.${year}' is invalid in parseReleaseDate() [3]`,
            );
            return;
        }
